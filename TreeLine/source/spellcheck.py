...
class SpellCheckOperation:
    ...
    def __init__(self, controlRef):
        ...
        self.lineNum = 0
        self.textLine = ''
        parentWidget = QtGui.QApplication.activeWindow()
        path = globalref.miscOptions.getValue('SpellCheckPath')
        while True:
            try:
                ...
                if path:
                    ...
                    if sys.platform.startswith('win'):
                        prompt = (_('Could not find either aspell.exe, '
                                    ...
                                    'Browse for location?'))
                        ans = QtGui.QMessageBox.warning(parentWidget,
                                                      _('Spell Check Error'),
                                                      prompt,
                        ...
                        title = _('Locate aspell.exe, ipsell.exe or '
                                  'hunspell.exe')
                        path = QtGui.QFileDialog.getOpenFileName(parentWidget,
                                                          title, '',
                                                          _('Program (*.exe)'))
                        if path:
                            path = path[:-4]
                        ...
                        QtGui.QMessageBox.warning(parentWidget, 'TreeLine',
                                                  prompt)
                        raise
    ...
    def spellCheck(self):
        ...
        if result and origBranches[0].parent:
            ...
            ans = QtGui.QMessageBox.information(parentWidget,
                                                _('TreeLine Spell Check'),
                                                prompt,
                                                QtGui.QMessageBox.Yes |
            ...
            if ans == QtGui.QMessageBox.Yes:
                ...
                result = False
        if result:
            QtGui.QMessageBox.information(parentWidget,
                                          _('TreeLine Spell Check'),
...
class SpellCheckDialog(QtGui.QDialog):
    """Dialog to perform and control the spell check operation.
    """
    misspellFound = QtCore.pyqtSignal()
    changeRequest = QtCore.pyqtSignal(str)
    ...
    def startSpellCheck(self, textLineIter):
        ...
        try:
            ...
            return True
        if self.spellCheck():
            if self.exec_() == QtGui.QDialog.Rejected:
                return False
        return True
